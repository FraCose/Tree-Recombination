...
#  
#  - mu*, sigma*, SIGMA* are the functions relative to the Toy Model considered in the reference.
# 
...
def build_tree(x_0, mu, SIGMA, n, 
        T=1, eps=1., FIG=False, DEBUG = False):
    
    if FIG: 
        plt.ion()
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.title('Grid building, n = %i, X | Y,' % n)

    gamma = np.sqrt(eps/(3.*n))
...
def sigma_fun(x):
    ...
    sigma[:,0,0] = np.cos(x[:,1])+2.
    sigma[:,1,1] = np.sin(x[:,0])+2.
    return sigma
...
def SIGMA_func(x):
    ...
    evalues[:,0] = (np.cos(x[:,1])+2)**2
    evalues[:,1] = (np.sin(x[:,0])+2)**2
    
    var_reshaped = np.zeros((len(x),3))
    var_reshaped[:,0] = (np.cos(x[:,1])+2.)**2
    var_reshaped[:,1] = (np.sin(x[:,0])+2.)**2
...
# uses the GPU
def sigma_fun_cupy(x):
    sigma = cp.zeros((len(x),2,2))
    sigma[:,0,0] = cp.cos(x[:,1])+2.
    sigma[:,1,1] = cp.sin(x[:,0])+2.
    return sigma

# uses the CPU
def simulate_toy(x_0, n, T,
                N=True):
    if N: N = n*n
...
if __name__ == 'toy_simulation':
    ...
    simulate_toy(x_0, n, T)
    print('time cpu = ', timeit.default_timer()-tic)

    x_0 = cp.array([0.,0.])

    tic = timeit.default_timer()
    simulate_toy_cupy(x_0, n, T)
